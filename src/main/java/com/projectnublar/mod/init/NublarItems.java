...
public class NublarItems {
	...
	public static void registerItemRenders()
	{
		...
		for(Iterator<Item> i = keys.iterator(); i.hasNext();)
		{
			...
			if(currentItem.getHasSubtypes()) {
				List<ItemStack> subItems = new ArrayList<>();
				currentItem.getSubItems(currentItem, currentItem.getCreativeTab(), subItems);
				subItems.stream()
					.forEach(itemStack -> registerRender(itemStack.getItem(), itemStack.getMetadata()));
			}
		}
	}
